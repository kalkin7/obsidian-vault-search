...
def test_sparse_results_append_sources_without_moving_direct_results(tmp_path: Path):
    ...
    try:
        ...
        results = expand_wiki_sources(
            connection, cfg, direct, "전체 경과", "timeline", 40, True, ["전체", "경과"], "any")
    finally:
        connection.close()

    assert [(item["file_path"], item["rank"], item["score"])
            for item in results[:20]] == original
    assert [item["file_path"] for item in results[20:]] == sources
    assert [item["rank"] for item in results[20:]] == [21, 22]
    assert all(item["expanded"] and item["source"] == "wiki_sources"
               for item in results[20:])
    assert all(item["channels"] == ["wiki_sources"] and item["rrf_contributions"] == {}
               for item in results[20:])
...
def test_sources_are_bounded_to_frontmatter_scope_and_existing_index(tmp_path: Path):
    ...
    try:
        results = expand_wiki_sources(
            connection, cfg, _direct(10, wiki), "연결", "timeline", 40,
            False, ["연결"], "any")
    finally:
        connection.close()
