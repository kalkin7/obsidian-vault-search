...
def call_runtime(
    vault: Path, method: str, params: dict[str, Any], timeout: float
) -> dict[str, Any]:
...
def _backend_root_for(vault: Path) -> Path:
    ...
    """
    plugin_backend = (
        vault / ".obsidian" / "plugins" / "obsidian-vault-search" / "backend"
    )
...
def _spawn_standalone(
    vault: Path, timeout: float, idle_exit_seconds: float
) -> dict[str, Any]:
    ...
    if not Path(python).exists():
        ...
        # via PATH before rejecting the spawn.
    ...
    try:
        ...
        raise ServiceUnavailable(f"Cannot open backend log {log_path}: {exc}") from exc
    ...
    while time.monotonic() < deadline:
        ...
        if (
            runtime is not None
            and runtime.get("vault_id") == vault_id(vault)
            and _pid_is_running(runtime.get("pid", 0))
            and _runtime_is_valid(vault)
        ):
            # Some Python distributions (e.g. certain venv builds on Windows)
            # ship a launcher python.exe that starts the real interpreter under
            # a *new* pid, so runtime.pid never equals the pid Popen returned.
            # A live pid with the right vault id, validated end-to-end via the
            # authenticated health probe, is sufficient to adopt the runtime.
            return _read_runtime(vault)
        if process.poll() is not None:
            raise ServiceUnavailable(
...
def _ensure_search_runtime(
    vault: Path, timeout: float, idle_exit_seconds: float, no_start: bool
) -> None:
...
def make_parser() -> argparse.ArgumentParser:
    ...
    for name, method in (
        ("rebuild-vectors", "rebuild_vectors"),
...
def main() -> None:
    ...
    try:
        ...
        if args.command in {"search", "rebuild-vectors", "rebuild-all"}:
            _ensure_search_runtime(
                vault, args.startup_timeout, args.idle_exit_seconds, args.no_start
            )
        ...
        if args.command in {"rebuild-vectors", "rebuild-all"}:
            ...
            print(
                json.dumps(
                    response.get("data") if response.get("ok") else response,
                    ensure_ascii=False,
                    indent=2,
                )
            )
...
def _print_error(error: dict[str, Any]) -> None:
    ...
    if code == "INDEX_REBUILD_REQUIRED":
        ...
        if action == "rebuild_vectors":
            print(
                "복구 방법: 벡터 재구축 — Obsidian 설정에서 '벡터 재구축'을 실행하거나"
                " `vault-search rebuild-vectors` 를 사용하세요.",
                file=sys.stderr,
            )
        elif action == "rebuild_all":
            print(
                "복구 방법: 전체 재구축 — Obsidian 설정에서 '전체 재구축'을 실행하거나"
                " `vault-search rebuild-all` 을 사용하세요.",
                file=sys.stderr,
            )
