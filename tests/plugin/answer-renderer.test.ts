...
function makeEl(): FakeEl {
	const element: FakeEl = {
		...
		createEl(tag: string, options?: Record<string, unknown>): FakeEl {
			...
			child.attrs = {
				...(options?.attr as Record<string, string> | undefined),
			};
...
describe("AnswerRenderer", () => {
	...
	it("renders task-list markers as read-only checkboxes, not literal [ ]", () => {
		const container = makeEl();
		const renderer = new AnswerRenderer(container as unknown as HTMLElement, {
			openCitation: async () => undefined,
		});
		renderer.render(
			["- [ ] 사전 검토 항목", "- [x] 제출 서류 항목", "- 일반 불릿"].join("\n"),
			[],
		);
		// Task items and plain bullets share one list (same as Obsidian).
		const lists = find(container, (node) => node.tag === "ul");
		expect(lists).toHaveLength(1);
		expect(
			lists[0].children.filter((node) => node.tag === "li"),
		).toHaveLength(3);
		const checkboxes = find(container, (node) => node.tag === "input");
		...
		expect(checkboxes[0].attrs["checked"]).toBeUndefined();
		expect(checkboxes[1].attrs["checked"]).toBe("checked");
		// The literal "[ ]" marker never reaches the text.
		const text = find(container, (node) => node.tag === "span")
			.map((node) => node.textContent)
			.join("");
		expect(text).not.toContain("[ ]");
		expect(text).toContain("사전 검토 항목");
		expect(text).toContain("제출 서류 항목");
	});
