...
> **기준일 주의 (2026-08-13)**: 이 문서는 2026-08-11 시점의 설계 원칙을 기록한 역사 문서다.
> 그 후 standalone sidecar가 도입되어 **“Obsidian이 sidecar 생명주기를 소유한다”와 “CLI가 별도 daemon을
> 시작하지 않는다”는 원칙은 더 이상 현행이 아니다.** CLI는 Obsidian 미실행 시 standalone backend를
> 자동 시작하고, 플러그인은 healthy standalone에 attach한다(단일 writer는 ServiceLock으로 유지).
> 아래 §2.1의 해당 항목들은 당시 기준으로 읽어야 한다.
>
