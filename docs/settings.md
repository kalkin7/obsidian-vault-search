...
- **전송 방식 2종**을 지원합니다.
  ...
  - **원격 URL(HTTP)**: 스트리밍 HTTP MCP 서버에 직접 연결. 서비스에서 발급한
    전체 접속 URL(토큰이 쿼리나 경로에 포함된 형태)을 모달에서 입력합니다.
    전체 URL은 **Obsidian `secretStorage`에만** 안전하게 저장되며, 설정 파일(`data.json`, `service-config.json`),
    상태 목록, 로그에는 안전한 오리진(`scheme://hostname[:port]`)만 노출됩니다.
    원격 서버에는 어떤 로컬 환경 변수도 주입되지 않습니다. OAuth 플로우는 미지원입니다.
- **보안 저장소(Secret Storage) 라이프사이클**:
  - 로컬 stdio 서버의 환경 변수 값과 원격 HTTP 서버의 전체 URL은 **Obsidian `secretStorage`에만** 저장됩니다.
  - 모달 편집 시 입력한 값은 메모리에 임시 보관(스테이징)되며, "저장" 버튼을 눌렀을 때만 보안 저장소에 반영됩니다. 취소하거나 모달을 닫으면 변경 사항이 버려지므로 고아 시크릿이 남지 않습니다.
  - sidecar 프로세스에는 인증된 loopback 프로토콜(`set_mcp_secrets`)을 통해 필요한 시크릿만 전달되며, 요청·응답·로그·예외 메시지에 비밀값이 절대 노출되지 않습니다.
  - v0.1.59~0.1.63 버전에서 평문으로 저장된 원격 URL은 플러그인 로드 시 보안 저장소로 자동 이전되고 설정 파일은 안전한 오리진으로 정리됩니다. 보안을 위해 원격 서비스의 토큰을 재발급(회전)하는 것을 권장합니다.
  - 전체 payload는 32 KiB, 이름 128자, 값 8 KiB, URL 2,048자로 제한됩니다. 서버 삭제 시 해당 시크릿도 함께 영구 삭제됩니다.
