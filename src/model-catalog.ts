...
// The models endpoint also lists dated snapshot ids (gpt-4o-2024-08-06). The
// undated alias (gpt-4o) always resolves to the newest snapshot, so dated ids
// only clutter the model picker.
const OPENAI_DATED_SNAPSHOT = /-\d{4}-\d{2}-\d{2}$/;

export function isSelectableAnswerModel(
  ...
  if (
    !normalized ||
    OPENAI_NON_CHAT_MARKERS.some((marker) => normalized.includes(marker))
  ) {
    return false;
  }
...
export function chooseProviderModel(
  ...
  if (availableModels.length) {
    return availableModels.includes(remembered)
      ? remembered
      : availableModels[0];
  }
